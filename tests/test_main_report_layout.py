...
def test_main_report_uses_requested_column_order(monkeypatch, tmp_path):
    ...
    assert worksheet.cell(row=3, column=2).value == "Found Source [Automatically]"
    assert worksheet.cell(row=3, column=3).value == "Remark"
    assert worksheet.cell(row=3, column=4).value == "Entity Name: Sales"
    assert worksheet.cell(row=3, column=5).value == "Level 1"
    assert 24 <= worksheet.column_dimensions["C"].width <= 60
